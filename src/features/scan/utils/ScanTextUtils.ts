...
export const extractPrice = (line: string): number | null => {
  ...
  const intlMatch = cleaned.match(/\b(\d{1,3}(?:,\d{3})+)(?:\.\d{1,2})?\b/)
  if (intlMatch) {
...
const extractLargestPrice = (line: string): number | null => {
  ...
  const idMatches = cleaned.matchAll(/\b(\d{1,3}(?:\.\d{3})+)(?:,\d{1,2})?\b/g)
  ...
  for (const m of intlMatches) found.push(parseInt(m[1].replace(/,/g, ""), 10))
