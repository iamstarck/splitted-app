...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!apiKey) {
      return res
        .status(500)
        .json({ error: "MINDEE_API_KEY is not configured in .env" })
    }
    ...
    if (fields.has("line_items")) {
      ...
      if (
        ...
        items = lineItemsField.items.map((item: any) => {
          ...
          const description =
            subFields.get?.("description")?.value ||
            subFields.description?.value ||
            subFields.get?.("item_description")?.value ||
            "Item"

          const totalAmount =
            subFields.get?.("total_amount")?.value ||
          ...
          const quantity =
            subFields.get?.("quantity")?.value || subFields.quantity?.value || 1
    ...
    console.log(
      `[OCR] Parsed ${items.length} items. Total: ${totalAmount}, Tax: ${totalTax}`
    )
